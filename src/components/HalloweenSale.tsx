...
async function HalloweenSale() {
  const sale = await getActiveSaleByCouponCode(COUPON_CODES.HALLOWEEN);
  if (!sale?.isActive) {
    return null;
  }
  return (
    <div
      className="bg-gradient-to-r from-black to-orange-500 text-white px-6
          ...
          <div className="flex ">
            <div className="bg-white text-black py-2 px-6 rounded-full shadow-md transform hover:scale-105 transition duration-300">
              <span className="font-bold text-base">
                Use code: {""}
                <span className="text-orange-600">{sale.couponCode}</span>
              </span>
